...
class MyForm(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, title="Screenshot Tutorial",size=(600, 300) )
        ...
        self.posCtrl = wx.TextCtrl(panel, -1, "william hill casino", pos=(100, 10),size=(350,20) )  # создаем атрибут - окно Edit - текстовое
    ...
    def onPlacewindow(self, event):
        ...
        if btn == 'Get Place Window':
           ...
           print('place', place)
           with open('WindowPlace3.txt', 'w') as jsonfile: json.dump(place, jsonfile)
        # win32gui.SetWindowPlacement(hwnd,(0, 1, (-1, -1), (-1, -1), (1922, 0, 2720, 474)))
        if btn == 'Place window':
            with open('WindowPlace3.txt', 'r') as f:  # извлекаем  из файла
                place = json.load(f)
