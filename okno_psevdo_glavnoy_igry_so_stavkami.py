...
class MyFrame(wx.Frame):
   ...
   def onPlacewindow(self, name):
       ...
       with open('WindowPlace2_.txt', 'r') as f:  # извлекаем  из файла
        place = json.load(f)
   ...
   def Glavnaja(self):
       ...
       vhod=True
       cikl_of_start =0
       file_obj2 = open('chet01_psevdo.txt', 'a')
       file_obj3 = open('log_psevdo.txt', 'a')
      ...
      # while (vhod):
       ...
       name_of_file_real = str(nomer_txt_fila)+'cikl_'+str(cikl_of_start)+'_data_psevdo'+tme_name+'.txt'
       ...
       while ( steps < chicloVrach):
           ...
           tme = self.vremja_str()
           # name_log_p_cntr_2 = 'log__p_cntr_2_' + tme + '.bmp'
           # screen_control_2.SaveFile(name_log_p_cntr_2, wx.BITMAP_TYPE_BMP)
           #chicloVrach = 200
           start = clock()
           first0 = time.time()
           steps = steps + 1
           ...
           while (True):
               ...
               time.sleep(0.1)
################################################  start   screen_control
               screen_control_1 = self.ScreenShotChisla('coord_snapshot_control.txt')
               time.sleep(1)
               p_cntr_1 = screen_control_1.GetData()
               # name_log_p_cntr_1 = 'log__p_cntr_1_' + tme + '.bmp'
               # screen_control_1.SaveFile(name_log_p_cntr_1, wx.BITMAP_TYPE_BMP)
###############################################    end        screen_control
               ...
               if p_cntr_1 == p_cntr_2:
                   ...
                   if steps_vnutri == 4:
                       #screen_control_2 = self.ScreenShotChisla('coord_snapshot_control.txt')
                       p_cntr_2 = dataPic[37].GetData()
                       screen_control_1 = self.ScreenShotChisla('coord_snapshot_prodolj.txt')
                       time.sleep(0.1)
                       p_cntr_1 = screen_control_1.GetData()
                       # self.mousePos(dataMouse[2][1])
                       # self.leftClick()
                       print("dopolnitelnyi click_super")
                       continue
                   if steps_vnutri == 20:
                       # self.mousePos(dataMouse[2][1])
                       # self.leftClick()
                   ...
                   for i in range(len(dataPic)):
                       ...
                       if p == p2:
                           ...
                           if i == 37:
                               screen5 = self.ScreenShotChisla('coord_snapshot.txt')
                               p = screen5.GetData()
                               for k in range(len(dataPic)):
                                   time.sleep(0.05)
                                   p2 = dataPic[k].GetData()
                                   if p == p2:
                                       print("выпал номер", k)
                                       break
                           chislo_of = i
  ...
  ########################################## конец блока предсказания и ставок
